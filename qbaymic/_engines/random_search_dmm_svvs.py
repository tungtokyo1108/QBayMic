...
def _sample_config(rng,
    ...
    Returns
    -------
    dict with keys: K_max, nu, selection_prior, prune_threshold
    """
    return {
...
def _run_trial(X, true_labels, config, trial_seed):
    ...
    try:
        model  = DMM_SVVS_Variational_v2(**params)
        model.fit(X)
...
def random_search(X,
    ...
    # ── Header ────────────────────────────────────────────────────────────
    if verbose:
        print("\nDMM-SVVS v2 — Random Hyperparameter Search  (scored by ARI)")
        print("=" * 90)
...
def refit_best(X,
    ...
    for idx, seed in enumerate(all_seeds):
        ...
        try:
            m     = DMM_SVVS_Variational_v2(**params)
            m.fit(X)
            pred  = m.predict(X)
...
def _print_row_header():
    print(f"  {'#':>5}  {'ARI':>7}  {'NMI':>7}  {'K_est':>5}  {'sec':>5}  "
          f"{'K_max':>5}  {'nu':>8}  {'sel':>6}  {'prune':>8}  {'seed':>12}")
    print("  " + "-" * 88)
...
def save_results(search_result, path="random_search_dmm_svvs_results.json"):
    """
...
if __name__ == "__main__":
    ...
    # ── Save all results ──────────────────────────────────────────────────
    save_results(results, path="random_search_dmm_svvs_results.json")
