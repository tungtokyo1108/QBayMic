...
# use_trigamma_correction is fixed to False per the user's request.
FIXED_PARAMS = dict(
    zeta                    = 1.0,
    eta                     = 1.0,
    xi_1                    = 1.0,
    xi_2                    = 1.0,
    beta0                   = 30.0,   # standard QAVB initial inverse temperature
    s0                      = 1.0,    # full quantum start
    tol                     = 1e-4,
    ...
    use_trigamma_correction = False,  # FIXED per user request
)
...
def _run_trial(X, true_labels, config, trial_seed):
    ...
    try:
        model = DMM_SVVS_PennyLaneQAVB_v2(**params)
        model.fit(X)
...
def random_search(X,
    ...
    # ── Header ────────────────────────────────────────────────────────────
    if verbose:
        print(f"\nDMM-SVVS PennyLaneQAVB_v2 — Random Hyperparameter Search  "
              f"(scored by ARI)")
        print("=" * 100)
        ...
        print(f"  Fixed: {json.dumps(fixed_display)}")
        print("=" * 100)
        _print_row_header()
...
def refit_best(X,
    ...
    # delete all clusters on refit.  Cap at 0.15.
    ...
    for idx, seed in enumerate(all_seeds):
        ...
        try:
            m     = DMM_SVVS_PennyLaneQAVB_v2(**params)
            m.fit(X)
            pred  = m.predict(X)
...
def print_top_k(search_result, top_k=10):
    ...
    print(f"\nTop-{n_shown} configurations — PennyLaneQAVB_v2 (by ARI):")
...
def save_results(search_result, path=None):
    ...
    search_result : dict  — return value of random_search()
    path          : str or None
        Output file path.  Defaults to random_search_pennylaneqavb_v2_results.json
    """
    if path is None:
        path = "random_search_pennylaneqavb_v2_results.json"

    with open(path, "w") as fh:
        json.dump(search_result["all_results"], fh, indent=2)
...
if __name__ == "__main__":
    ...
    # ── Search ────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("RANDOM SEARCH: DMM_SVVS_PennyLaneQAVB_v2")
    print("=" * 60)
    results = random_search(
        X                     = X,
        true_labels           = true_labels,
    ...
    ari  = adjusted_rand_score(true_labels, pred)
    nmi  = normalized_mutual_info_score(true_labels, pred)
    print(f"  PennyLaneQAVB_v2   ARI={ari:.4f}  NMI={nmi:.4f}  "
