...
FIXED_PARAMS = dict(
    ...
    n_phi_warmup            = 5,       # used only by update_strategy="phi_first"
    mixer                   = "transverse_field",
    warm_start              = True,
    adaptive_expressivity   = True,   # K-adaptive depth/steps floor (v4 fix):
                                      # raises depth≥n_sys+1, steps≥~100 at high
                                      # s_t so the Gibbs-fidelity gap closes.
    enumerate_basis         = True,
    # ── Layer-C dedup (inherited from v4_1) ──────────────────────────────
    dedup_n_clusters        = None,   # per-sample path; "auto" for very large N
    # ── v4_2 JAX knobs ───────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Sampling helpers  (identical to v3_1 — kept local so the file stands alone)
# ─────────────────────────────────────────────────────────────────────────────
...
def _run_trial(X, true_labels, config, trial_seed):
    ...
    try:
        model = DMM_SVVS_VQT_QAVB_JAX_Fast(**params)
        model.fit(X)
...
def random_search(X,
    ...
    # ── Header ────────────────────────────────────────────────────────────
    if verbose:
        print(f"\nDMM-SVVS VQT_QAVB_JAX_Fast (v4_2fast) — Random Hyperparameter "
    ...
    return {
        ...
        "model_class":  "VQT_QAVB_JAX_Fast_v4_2",
    }
...
def refit_best(X,
    ...
    # delete all clusters on refit.  Cap at 0.15 (same convention as v2/v3_1).
    ...
    if verbose:
        print(f"\nFinal refit (VQT_QAVB_JAX_Fast v4_2fast) — "
    ...
    for idx, seed in enumerate(all_seeds):
        ...
        try:
            m     = DMM_SVVS_VQT_QAVB_JAX_Fast(**params)
            m.fit(X)
            pred  = m.predict(X)
...
def print_top_k(search_result, top_k=10):
    ...
    print(f"\nTop-{n_shown} configurations — VQT_QAVB_JAX_Fast v4_2fast (by ARI):")
...
def save_results(search_result, path=None):
    ...
    path          : str or None
        ...
        random_search_vqt_qavb_jax_fast_v4_2_results.json
    """
    if path is None:
        path = "random_search_vqt_qavb_jax_fast_v4_2_results.json"
...
if __name__ == "__main__":
    ...
    # ── Search ────────────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("RANDOM SEARCH: DMM_SVVS_VQT_QAVB_JAX_Fast (v4_2fast)")
    print("=" * 60)
    ...
    ari  = adjusted_rand_score(true_labels, pred)
    nmi  = normalized_mutual_info_score(true_labels, pred)
    print(f"  VQT_QAVB_JAX_Fast v4_2fast   ARI={ari:.4f}  NMI={nmi:.4f}  "
